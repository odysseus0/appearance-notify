...
gh auth status > /dev/null || {
  echo "gh is not authenticated" 2>&1
  exit 1
}

BRANCH=$(git rev-parse --abbrev-ref HEAD)
if [[ "${ALLOW_NON_MAIN:-0}" != 1 && "$BRANCH" != "main" ]]; then
  echo "Publish must run on main (current: $BRANCH). Set ALLOW_NON_MAIN=1 to override." 2>&1
  exit 1
fi
...
if [[ -f .release-state ]]; then
  ...
  [[ -n "$VERSION_NUM" ]] || { echo "Unable to determine version from $ver_file" 2>&1; exit 1; }
  TAG="v$VERSION_NUM"
fi
...
(echo "$VERSION_OUT" | grep -q "$VERSION_NUM") || { echo "Version mismatch: expected $VERSION_NUM" 2>&1; exit 1; }

# GitHub release
if [[ "$DRY_RUN" != 1 ]]; then
  gh release create "$TAG" dist/appearance-notify-apple-darwin.tar.gz -F <(git-cliff --tag "$TAG") -t "$TAG"
else
...
if [[ "$DRY_RUN" != 1 ]]; then
  ...
  if [[ "$origin_url" =~ github.com[:/]([^/]+)/([^/.]+)(\.git)?$ ]]; then
    ...
    echo "Warning: could not parse origin URL, defaulting OWNER/REPO placeholders" 2>&1
